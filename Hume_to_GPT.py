...
from pvrecorder import PvRecorder                       #remove?
from whispercpp import Whisper                          #remove?
from terminal_gpt import get_completion, get_completion_fast
from chat import message, store_emotions                #remove?
from playsound import playsound                         #remove?
from hume import HumeStreamClient, HumeClientException
from hume.models.config import FaceConfig               
from gtts import gTTS                                   #remove?


# Configurations
openai.api_key = ""
HUME_API_KEY = "T46iFimyuk6sPJqfdWz3thRyo3U3y5RbfH1H5T16FETzF5va"
HUME_FACE_FPS = 1  # 3 FPS
                    ...
                    if not recording:
                        ...
                        for emotion in NEGATIVE_EMOTIONS:
                            if not do_nothing and Hume_data[emotion] > THRESHOLD:
                                prompt += f"and {Hume_data[emotion] * 100}% of my students are experiencing the emotion of {emotion}, "
                                threshold_exceeded = True

                        if threshold_exceeded or test_prompting and not awaiting_option:
                            if test_prompting:
                                print(Hume_data['Boredom'] * 100)
                                prompt += f"and {Hume_data['Boredom'] * 100}% of my students are experiencing the emotion of Boredom, "
                            test_prompting = False
                            threshold_exceeded = False
...
def on_press(key):
    ...
    if key == keyboard.Key.right or key == keyboard.Key.space:
        ...
        #TODO: Add option to go back to previous subject
